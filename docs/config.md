...
- `SKYLIGHT_MCP_OAUTH_LOGIN` — legacy explicit switch for the built-in OAuth 2.1/PKCE browser login. It is no longer necessary because browser login is the default whenever no HTTP token or insecure mode is configured.
- `SKYLIGHT_OAUTH_DB_PATH` — persistent SQLite file required only for production hosted OAuth. Token, insecure-local, stdio, and local development modes do not initialize SQLite.
- `SKYLIGHT_OAUTH_MASTER_KEY` — one 32-byte base64url deployment secret required with the SQLite path. Independent encryption, subject, and signing keys are derived from it.

The built-in mode publishes authorization-server and protected-resource metadata, supports dynamic client registration and PKCE, and issues short-lived access tokens plus rotating refresh tokens. Local development uses explicit in-memory storage. Production fails closed without durable encrypted storage and stable signing and subject keys. Each verified subject receives only its own Skylight credential. SQLite supports many subjects in one process but is intentionally a single-machine deployment; do not attach the database volume to horizontally scaled instances. External OAuth verification is rejected because it cannot safely map an external subject to a Skylight account. Locally, `skylight-calendar-mcp-http` works without options at `http://127.0.0.1:8787/mcp`; only public deployments need bind, public URL, and durable-store settings.
